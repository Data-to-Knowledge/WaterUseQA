...
1. A spreadsheet containing a) a summary of the water use data that is
available in Hilltop, and b) monthly summary statistics.
2. A pdf file containing several time-series plots.
...
def remove_negative_values(dataframe):
    ...
    print("{0} negative values were removed from the water use dataset".format(neg_values))
    return vol_data2
...
def export_monthly_stats(measurements, statistics, site):
    ...
    measurements.to_excel(writer, sheet_name='WaterUseData')
    worksheet = writer.sheets['WaterUseData']
    worksheet.set_column('B:I', 20)
    # Export monthly statistics
    statistics.to_excel(writer, sheet_name='MonthlySummary')
...
def main():
    ...
    print("Calculating monthly water use statistics")
    ...
    export_monthly_stats(mslist2, monthly_stats, site)
    print("Monthly water use statistics have been exported to Excel")
    ...
    print("Time series plots have been exported to PDF")
